...
class FeedForwardNN(nn.Module):
    def __init__(self, in_dim_1, in_dim_2, out_dim, type):
        ...
        if type == "actor":
            ...
            self.actor2 = nn.Linear(128, out_dim)

        elif type == "critic":
            # Critic Layers
            self.action1 = nn.Linear(1, 8)
            self.action2 = nn.Linear(8, 32)

            self.state1 = nn.Linear(224, 128)  # 128+64+32
    ...
    def forward(self, obs_radar, obs_state, obs_action):
        ...
        if self.type == "actor":
            ...
            output = F.sigmoid(self.actor2(act1))
