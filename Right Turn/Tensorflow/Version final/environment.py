...
class CarlaVehicle(object):
	...
	def reset(self,Norender,loc):
		...
		# self.target_waypoint = carla.Transform(carla.Location(x = 1.89, y = 117.06, z=0), carla.Rotation(yaw=269.63))
		self.target_waypoint = carla.Transform(carla.Location(x = 2.1, y = 116.06, z=0), carla.Rotation(yaw=269.63))
		...
		self.radar.set_attribute("horizontal_fov", f"45")
		self.radar.set_attribute("vertical_fov", f"25")

		#We will initialise Radar Data
		self.resetRadarData(100, 45, 25)
	...
	def step_straight(self, action, p):
		...
		return data[-LIMIT_RADAR:], [kmh, norm_target] , reward, done, None
	'''
	...
	def step_rightturn(self, action, debug):
		...
		if norm_target<=3 and diff_angle<=0.1:
			done = True
			print('target reached')
			reward = reward+500
		else:
			#More close to the target better the reward
			reward += 0.2*(90-diff_angle) + 0.5*(-norm_target)
		# angle 90----0 norm 20----2
	...
	# ==============================================================================
	# -- Left Lane Change DDPG Network ------------------
	# ==============================================================================
	'''
	def step_leftlanechange(self,action,debug):
		done = False
		#Calculate vehicle speed
		self.vehicle.apply_control(carla.VehicleControl(throttle=action[0], steer=action[1], brake=action[2], reverse=action[3]))
		kmh = self.get_speed()
		...
		return data[-LIMIT_RADAR:],[kmh,self.yaw_vehicle,self.lane_id_ego,abs(abs(int(179+self.yaw_vehicle))-abs(int(self.yaw_target_road)))] , reward, done, None
	'''
