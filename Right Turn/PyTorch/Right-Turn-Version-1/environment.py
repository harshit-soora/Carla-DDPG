...
class CarlaVehicle(object):
	...
	def reset(self,Norender,loc):
		...
		#right turning agent
		x = random.uniform(154.5, 158.5)
		init_loc = carla.Location(x = x, y = -182, z=1)
		init_pos = carla.Transform(init_loc, carla.Rotation(yaw=270))
		# self.target_waypoint = carla.Transform(carla.Location(x = 1.89, y = 117.06, z=0), carla.Rotation(yaw=269.63))
		r =  random.uniform(0, 2)
		self.target_waypoint = carla.Transform(carla.Location(x = 170, y = -196 + r, z=0), carla.Rotation(yaw=359.9))
	...
	def step_rightturn(self, action, debug):
		...
		if norm_target<=3 and diff_angle<=1:
			done = True
			print('target reached')
			reward = reward+100
		else:
			#More close to the target better the reward
			reward += 0.05*(90-diff_angle) + 0.2*(-norm_target)
