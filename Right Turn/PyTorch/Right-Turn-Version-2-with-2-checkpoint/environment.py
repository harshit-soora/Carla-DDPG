...
class CarlaVehicle(object):
	...
	def step_rightturn(self, action, debug):
		...
		check_1.x = init_location.x + rad * math.cos(pie/6)
		check_1.y = init_location.y - rad * math.sin(pie/6) 
		...
		print(f"Tar--{norm_target}---1--{norm_1}---2--{norm_2}")
		...
		if norm_target<=1.5 and diff_angle<=0.5:
			...
			reward += 0.1*(90-diff_angle)

			if diff_angle>60:
				val = 0.7*(-norm_1) + 0.2*(-norm_2) + 0.1*(-norm_target)
			elif diff_angle>30:
				val = 0.8*(-norm_2) + 0.2*(-norm_target)
			else:
				val = (-norm_target)
	...
	def compute_magnitude_angle(self, target_location, current_location, target_yaw, current_yaw):
		...
		return (norm_target, abs(current_yaw) % 180)
