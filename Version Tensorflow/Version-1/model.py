...
# Own Tensorboard class for logging,  taken form youtube channel :: sentdex
...
class DDPG:
	...
	def __init__(self, action_space, state_space, type):
		...
		self.critic_lr = self.lr + 0.001 
		self.actor_lr = self.lr

		self.critic_optimizer = tf.keras.optimizers.Adam(self.critic_lr)
		self.actor_optimizer = tf.keras.optimizers.Adam(self.actor_lr)

		self.first_layer_actor = 64
		self.second_layer_actor = 64
		self.first_layer_critic = 128
		self.second_layer_critic = 64
	...
	def get_actor(self):
		...
		out1 = layers.Dense(self.first_layer_actor, activation="relu")(inputs)
		out2 = layers.BatchNormalization()(out1)
		out3 = layers.Dense(self.second_layer_actor, activation="relu")(out2)
		out4 = layers.BatchNormalization()(out3)

		outputs = layers.Dense(self.action_space, activation="sigmoid", kernel_initializer=last_init)(out4)
	...
	def get_critic(self):
	    ...
	    action_out0 = layers.Dense(8, activation="relu")(action_input)
	    action_out1 = layers.Dense(16, activation="relu")(action_out0)
	    action_out2 = layers.BatchNormalization()(action_out1)

	    # Both are passed through seperate layer before concatenating
	    concat = layers.Concatenate()([state_out4, action_out2])

	    out1 = layers.Dense(self.first_layer_critic, activation="relu")(concat)
	    out2 = layers.BatchNormalization()(out1)
	    out3 = layers.Dense(self.second_layer_critic, activation="sigmoid")(out2)
	    out4 = layers.BatchNormalization()(out3)

	    last_init = tf.random_uniform_initializer(minval=-0.005, maxval=0.005)
	    outputs = layers.Dense(1,kernel_initializer=last_init)(out4)
	    # Critic Outputs the Q values, it uses no activation, we use its raw value
