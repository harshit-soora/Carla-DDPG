...
def train_straight_DDPG(episodes,agent):
	...
	for epi in range(episodes):
		try:
			...
			start_state = [0,round(agent.get_location().y, ROUNDING_FACTOR)]
			...
			straight_model.update_target(0.005)
			...
			straight_model.tensorboard.update_stats(reward_avg=avg_reward, reward_min=np.mean(critic_loss_epi), reward_max=np.mean(actor_loss_epi))
			if(epi%20==0):
				x_label = 'Episodes'
				y_label = 'Actor Loss'
