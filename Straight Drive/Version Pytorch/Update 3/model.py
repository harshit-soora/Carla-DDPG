...
class DDPG:
	...
	def __init__(self, action_space, state_space, radar_dim, type):
		...
		self.t_so_far = 0
		now = time.localtime()
		self.writer = SummaryWriter(log_dir=f"runs/Jan-{now.tm_mday}_{tm_min}:{tm_hour}_{self.radar_space}_{self.critic_lr}_{self.batch_size}")
	...
	def policy(self, radar_state, physical_state):
		...
		legal_action = np.clip(sampled_actions.detach().numpy(), self.lower_bound, self.upper_bound)
	...
	def replay(self):
		...
		for name, param in self.actor.named_parameters():
			if 'layer' in name:
				new_dict[name] = (param.data + new_dict[name])/2
