...
class CarlaVehicle(object):
	...
	def reset(self,Norender,loc):
		...
		self.init_pos = carla.Transform(init_loc, carla.Rotation(pitch=0.094270, yaw=-3.097657, roll=-1.246277))
	...
	def left_lane_change(self,action):
		...
		vehicle_waypoint = self.map.get_waypoint(self.vehicle.get_location())
		self.lane_id_ego = vehicle_waypoint.lane_id
		self.lane_id_target = self.next_lane_target.lane_id
		self.yaw_vehicle = vehicle_waypoint.transform.rotation.yaw
		...
		if (self.lane_id_ego!=self.lane_id_target):
			...
			if len(self.collision_hist) != 0:
				...
				print(f'COLLISION HISTORY========={self.collision_hist}====len{len(self.collision_hist)}')
				reward = reward-1000
			if(abs(abs(int(179+self.yaw_vehicle))-abs(int(self.yaw_target_road)))<=3):
				done = True
				print('target reached')
				reward = reward+500
		...
		return data[-LIMIT_RADAR:], [kmh, self.yaw_vehicle, lane, diff_angle] , reward, done, None
